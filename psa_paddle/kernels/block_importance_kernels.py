"""
Block Importance Kernels for PaddlePaddle
...
def softmax_fuse_block_sum_kernel_causal(
    ...
    input_ptr = input_ptr + tl.arange(0, segment_size) + tl.arange(0, block_size)[
        :, None
    ] * input_stride_2
...
def flat_group_gemm_fuse_reshape_kernel(
    ...
    Q_ptrs = Q_ptrs + tl.arange(0, BLOCK_M)[:, None] * (stride_qn * STRIDE) + tl.arange(0, HEAD_DIM)[
        None, :
    ] + stride_qn * (STRIDE - 1)
    K_ptrs = K_ptrs + tl.arange(0, BLOCK_N)[None, :] * (stride_kn * STRIDE) + tl.arange(0, HEAD_DIM)[
        :, None
    ]
...
def softmax_fuse_block_sum(
    ...
    """
    Softmax fused with block sum for PaddlePaddle tensors.
    """
    ...
    assert attn_weights_slice.strides[-1] == 1

    output = paddle.empty(
        shape=[batch_size, num_heads, q_len // reshaped_block_size, k_len // reshaped_block_size],
    ...
    if is_causal:
        softmax_fuse_block_sum_kernel_causal[grid](
            ...
            output,
            scale,
            attn_weights_slice.strides[0],
            attn_weights_slice.strides[1],
            attn_weights_slice.strides[2],
            output.strides[0],
            output.strides[1],
            output.strides[2],
            real_q_len,
...
def flat_group_gemm_fuse_reshape(
    ...
    """
    Flat group GEMM with fused reshape for PaddlePaddle tensors.
    """
    ...
    output = paddle.empty(
        shape=[batch_size, num_heads, q_len // stride, kv_len // stride],
    ...
    flat_group_gemm_fuse_reshape_kernel[grid](
        ...
        key_states,
        output,
        query_states.strides[0],
        query_states.strides[1],
        query_states.strides[2],
        key_states.strides[0],
        key_states.strides[1],
        key_states.strides[2],
        output.strides[0],
        output.strides[1],
        output.strides[2],
        chunk_start,
